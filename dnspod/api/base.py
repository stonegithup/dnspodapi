...
class RestApi(object):
    ...
    def __init__(self, domain, port = 80):
        ...
        if(dnspod.getDefaultAppInfo()):
            self.__access_key_id = dnspod.getDefaultAppInfo().accessKeyId
            self.__access_key_secret = dnspod.getDefaultAppInfo().accessKeySecret
    ...
    def set_app_info(self, appinfo):
        #=======================================================================
        # 设置请求的app信息
        # @param appinfo: import dnspod
        #                 appinfo dnspod.appinfo(accessKeyId,accessKeySecret)
        #=======================================================================
    ...
    def getResponse(self, authrize=None, timeout=30):
        ...
        # 获取response结果
        #=======================================================================
        connection = httplib.HTTPSConnection(self.__domain, self.__port)
        apiname_split = self.getapiname()
        parameters = { \
                'format'        : 'json', \
                'login_email'   : self.__access_key_id
        }
        application_parameter = self.getApplicationParameters()
        for key in application_parameter.keys():
            parameters[key] = application_parameter[key]

        parameters['login_password'] = self.__access_key_secret
        url = "/" + apiname_split 
